RaceSubList["something catlike-something great catlike"] = { // The name of the object must be the name of the parent ClassList entry ("something catlike" in this example), followed by a dash ("-"), followed by the name of the variant ("something great catlike" in this example), as defined in the 'variants' array in the ClassList entry (see bottom line). Note the use of only lower case! Also note the absence of the word "var" and the use of brackets []
	...
	// after defining the above four, you don't need to define anything more, but you can. Defining more stuff will overwrite the entries as they are given in the RaceList. So if you do not need something to be different than the basics of the class (for example, you subclass uses the same spellcasting ability), then you don't need to define it again.
	...
	speed : ["30 ft\n30 ft fly", "20 ft\n20 ft fly"], // change the speed to something else. Note that speed can be either just a number, or a string. Using just a number is faster. Only use a string when you need to add something more (like a fly speed)
	...
	features : "", //the background has features defined, but this way the variant offers no features
	
	abilitySave : 0, //the background has an Ability Save defined, but this way the variant offers no Ability Save
}
...
// if the race already has defined variants (as of v12.5 only the dragonborn, half-elf, human, and tiefling do) you should use the syntax below:
