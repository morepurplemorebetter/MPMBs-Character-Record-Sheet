var Base_CompanionList = {
	"familiar" : {
		...
		includeCheck : function(sCrea, objCrea, iCreaCR) {
			...
			return !!isDisplay("DCI.Text") && objCrea.companion === "familiar_not_al" ? " (if DM approves)" : false;
		},
		action : [
	...
	"pact_of_the_chain" : {
		...
		includeCheck : function(sCrea, objCrea, iCreaCR) {
			// return true if to be included, or a string to add a note to the menu option
			return objCrea.companion === "familiar" ? true : !!isDisplay("DCI.Text") && objCrea.companion === "familiar_not_al" ? " (if DM approves)" : false;
		},
		action : [
			["action", "Familiar (dismiss/reappear)"],
			["action", "Use familiar's senses"]
