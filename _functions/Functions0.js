...
function MakeRegex (inputString, extraRegex) {
  return RegExp('^(?=.*\\b' + inputString.replace(/\W/g, ' ').replace(/^ +| +$/g, '').RegEscape().replace(/('?s'?)\b/ig, '\($1\)?').replace(/ +/g, 's?\\b)(?=.*\\b') + 's?\\b)' + (extraRegex || '') + '.*$', 'i')
};
...
function toSup (inString) {
  ...
  var useCaps = true
  for (i = 0; i < input.length; i++) {
    useCaps = !(!useCaps || (/c|f|s|x|y|z/i).test(input[i]))
    output[i] = ''
