...
function AdventureLeagueOptions (MenuSelection) {
  MenuSelection = MenuSelection || getMenu('adventureLeague')
...
function AddArmor (armour, force, comp) {
  if (!armour) return
  var prefix = comp || !event.target || !event.target.name ? '' : getTemplPre(event.target.name, 'AScomp', true)
...
function ReCalcWeapons (justProfs) {
  ...
  IsNotReset = false
  justProfs = !!(justProfs && !forceReCalcWeapons && (!CurrentEvals.atkAdd || !(/level/i).test(CurrentEvals.atkAdd)))
...
function AddToInv (area, column, item, amount, weight, location, searchRegex, AddTestReplace, checkKey, isCorrectUnits) {
  ...
  searchRegex = searchRegex || MakeRegex(searchItem.replace(/\uFEFF|\,[^\,]+$/g, (/(\+|-)\d+/).test(searchItem) ? '' : '(?!.*(\\+|-)\\d+)'))
...
function AddInvWeaponsAmmo () {
  ...
  for (var i = 1; i <= FieldNumbers.attacks; i++) {
    ...
    if (theWeight && !theAtk[3] && !specialAtkName(theFld)) {
      var theTxt = (theWea || theFld) + theAtk[1]
...
function MakeInventoryLineMenu () {
  ...
  var menuLVL1 = function (menu, array) {
    for (var i = 0; i < array.length; i++) {
      var isEnabled = !((array[i][1] === 'up' && !upRow) || (array[i][1] === 'down' && !downRow) || (!theField && (/move|insert/i).test(array[i][1])))
      menu.push({
        cName: array[i][0],
  ...
  var AddCompOptions = function (menu) {
    ...
    for (var i = 0; i < AScompA.length; i++) {
      ...
      temp.oSubMenu.push({
        cName: (CompNm || 'NAME') + "'s Equipment Section " + (eqpVis ? '' : '\[not visible currently\] ') + '(page ' + CompPg + ')',
...
function AddLangTool (typeLT, input, tooltip, uniqueID, replaceThis, replaceMatch) {
  ...
  var theSubmit = uniqueID || inputCl
...
function AddString (field, inputstring, newline) {
  ...
  var separator = (newline !== true && newline !== false && thefield.value !== '') ? (newline || ' ') : ''
...
function ApplyProficiencies (updatefields) {
  ...
  if (updatefields) {
    ...
    Checkbox('Proficiency Weapon Other', (WeaponOtherString.length > 0), WeaponOtherTip)
...
function AddFeature (identifier, usages, additionaltxt, recovery, tooltip, UpdateOrReplace, Calc) {
  ...
  UpdateOrReplace = UpdateOrReplace || 'replace'
      ...
      for (var prop in temp.features) {
        ...
        if (Typeswitch !== 'proficiencies' && ClassLevelUp[aClass][0] !== 'stop') {
          ...
          if (propFea.extrachoices && propFea.minlevel <= ClassLevelUp[aClass][2]) {
            ...
            if (addRemoveEC) {
              propFea.extrachoices.forEach(function (extraF) {
...
function ClassFeatureOptions (Input, inputRemove, useLVL) {
  tDoc.delay = true
  tDoc.calculate = false
  var MenuSelection = Input || getMenu('classfeatures')
  AddOrRemove = inputRemove || (MenuSelection && MenuSelection[4] ? MenuSelection[4] : '')
  var tempArray = []
...
function ApplyColorScheme (aColour) {
  ...
  for (var a = 1; a <= FieldNumbers.attacks; a++) {
    ...
    if (a <= 3) {
      for (var pA = 0; pA < prefixAScomp.length; pA++) {
...
function MakeWeaponMenu () {
  ...
  var isEquipment = !!(QI && What('BlueText.Attack.' + itemNmbr + '.Weight') && (What('Manual Attack Remember') === 'Yes' || isWeapon))
...
function ApplyAttackColor (attackNmbr, aColour, type, prefix) {
  ...
  startNmbr = attackNmbr || 1
  endNmbr = attackNmbr || maxItems
