...
function DirectImport (consoleTrigger) {
  ...
  } else if (global.docFrom && global.docTo) {
    try { // we are good to go and import stuff!
  ...
  // >> make a function to do all children of a parent field
      var doChildren = function (parentFld, fromPre, toPre, excludeRegEx, inclVisibility, actionsObj) {
        ...
        for (var pA = 0; pA < parentA.length; pA++) {
          ...
          ImportField(pAnameTo, actionsObj || {notTooltip: true, doVisiblity: inclVisibility}, pAnameFrom)
        }
      }
...
function ImportField (fldNm, actionsObj, fromFldNm) {
  ...
  if (!global.docTo || !global.docFrom || !global.docTo.getField(fldNm) || !global.docFrom.getField(fromFldNm || fldNm)) return false // either of the documents or fields doesn't exist

  var didChange = false
  var toFld = global.docTo.getField(fldNm)
  var fromFld = global.docFrom.getField(fromFldNm || fldNm)
...
function RunUserScript (atStartup, manualUserScripts) {
  ...
  var manualScript = manualUserScripts || What('User Script')
  if (manualScript) {
...
function ImportScriptOptions (input) {
  var MenuSelection = input || getMenu('importscripts')
