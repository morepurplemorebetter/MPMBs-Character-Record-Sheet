  ...
  for (var aClass in CurrentClasses) {
    for (var pop in CurrentClasses[aClass].features) {
      ...
      if ((/armor of peace|unarmored defense|draconic resilience|durability/i).test(fea.name) && (/ AC /).test(fea.description) && fea.minlevel <= classes.known[aClass].level) {
        ...
        if (addAbi) {
          for (var aA = 0; aA < addAbi.length; aA++) {
    ...
    if (!enabled) {
      ...
      for (var i = 0; i < array.length; i++) {
        ...
        temp.oSubMenu.push({
          ...
          bMarked: subMarked,
          bEnabled: array[i][1] === 'no-mm' ? false : !(name[1] === 'change' && (array[i][1] === 'companion' || array[i][1] === 'companionrr') && CurrentCompRace[prefix] && CurrentCompRace[prefix].typeFound !== 'creature')
        })
      }
    }
    menu.push(temp)
...
function ChangeFont (newFont, oldFont) {
  newFont = newFont || (!typePF ? 'SegoePrint' : 'SegoeUI')
  oldFont = oldFont || tDoc.getField((tDoc.info.AdvLogOnly ? 'AdvLog.' : '') + 'Player Name').textFont
...
function DoTemplate (tempNm, AddRemove, removePrefix, GoOn) {
  ...
  return theNewPrefix || '' // if a new template was created with a prefix, return that prefix
};
...
function UpdateLogsheetNumbering (prefix, prePrefix) {
  prePrefix = prePrefix || CalcLogsheetPrevious(prefix)
...
function UpdateALdateFormat (dateForm) {
  dateForm = dateForm || What('DateFormat_Remember')
...
function MakeSkillsMenu_SkillsOptions (input) {
  ...
  var MenuSelection = input || getMenu('skills')
...
function ApplyWeapon (inputText, fldName, isReCalc, onlyProf) {
  if (IsSetDropDowns) return // when just changing the dropdowns, don't do anything
  fldName = fldName || event.target.name
      ...
      description: {
        ...
        elements: [{
          ...
          elements: [{
            ...
            wrap_name: true,
            char_width: 35,
            name: theName || "Set the field's value"
          }, {
            type: 'cluster',
            alignment: 'align_fill',
            item_id: 'txt0',
...
function AddToModFld (Fld, Mod, Remove, NameEntity, Explanation) {
  ...
  if (!isNaN(Mod)) {
    ...
    if (Remove) {
      ...
      setFld = (aFld || '') + (Mod.substr(0, 1) === '-' ? '' : '+') + Mod
    };
  };
  ...
  switch (ProfType) {
    ...
    // now update the resistance fields
      ...
      // now add the resistance
        ...
        RemoveResistance(Extra || ProfObj)
        resRemoved = 1
      };
    ...
    commit: function (dialog) {
      ...
      for (var i = 0; i < this.subj.length; i++) {
        ...
        if (this.already === 'radio') {
          ...
          this.choices.push(theResult || this.subj[i])
        };
      };
    },
