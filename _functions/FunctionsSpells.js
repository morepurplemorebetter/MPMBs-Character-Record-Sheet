...
function ApplySpell (FldValue, rememberFldName) {
  ...
  var base = rememberFldName || event.target.name
  var isPF = typePF
...
function SetSpell (FldValue, nameFldName) {
  ...
  var input = FldValue || What(event.target.name)
  var base = nameFldName || event.target.name
...
function AskUserNumber (caption) {
  var theDialog = {
    ...
    description: {
      name: caption || 'Amount of empty rows to insert',
      elements: [{
        type: 'view',
        align_children: 'align_left',
        elements: [{
...
function HideSpellSheetElement (theTarget) {
  var base = theTarget || event.target.name
  var prefix = base.substring(0, base.indexOf('spells'))
...
function AddSpellSheetTextLines (prefix, boxes, maxLine) {
  maxLine = maxLine || FieldNumbers.spells[1]
